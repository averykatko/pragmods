...
var participant_response_type = 3;//OVERSPEC
...
var linguistic_framing = 13;//OVERSPEC
...
var referent_to_describe = random(0,2);

var stim_1 = random(0,stims.length-1);
var stim_2 = random(0,stims.length-1);
var stimuli_to_show = [stim_1,stim_2];///range(0, 6);
...
var sequence_condition = 0;//random(0,1);///1 for 0w,1w,1b; 0 for 1w,0w,1b
if(0 === sequence_condition)
{
    target_unpermuted_for_trial = [1,1];
    distractor_unpermuted_for_trial = [2,2];
    //choice_names_unpermuted_for_trial = [["foil","target","logical"],["foil","logical","target"],["foil","target","logical"]];
    target_prop_unpermuted_for_trial = [2,2];
    distractor_prop_unpermuted_for_trial = [1,1];
    participant_response_type_for_trial = [3,0];//virtual keyboard, forced choice
    linguistic_framing_for_trial = [13,1];//keyboard, one word
}
else
{
...
var get_next_trial_data = function(){
    ...
    participant_response_type = participant_response_type_for_trial[trials_completed];
    linguistic_framing = linguistic_framing_for_trial[trials_completed];
}

var permute_before_trial = [1,1];///whether to permute the matrix before trial n
...
var num_trials = 2;///formerly hardcoded as 6 in control flow code
