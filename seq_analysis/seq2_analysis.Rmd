...
for (p in d2$id) {
  ...
  for (t in 1:3) {
    cur.df <- subset(particip.data,trial==t)
    word.row <- c(1,2,3)[as.numeric(cur.df$level)+1]
    ...
    eref.mat[choice.idx(cur.df$choice,cur.df$level),]
